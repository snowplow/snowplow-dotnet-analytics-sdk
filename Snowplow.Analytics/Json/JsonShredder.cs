...
namespace Snowplow.Analytics.Json
{
    ...
    /// </summary>
    public static class JsonShredder
    {
        ...
        public static Func<string, JObject> ParseContexts = (contextJsons) =>
        {
            ...
            if (data == null)
            {
                throw new SnowplowEventTransformationException("Could not extract data field from custom context.");
            }
            ...
            foreach (var context in data)
            {
                ...
                try
                {
                    var schema = FixSchema(Constants.CONTEXT_KEY, context["schema"].Value<string>());
            ...
            {
                throw new SnowplowEventTransformationException(errors.ToList());
            }
        ...
        public static Func<string, JObject> ParseUnstruct = (unstruct) =>
        {
            ...
            if (data == null)
            {
                throw new SnowplowEventTransformationException("Could not extract data field from unstructured event.");
            }
            ...
            var fixedSchema = FixSchema(Constants.UNSTRUCT_EVENT_KEY, schema.Value<string>());
